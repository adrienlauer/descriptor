...
func CloneTemplateContext(other *TemplateContext, cr ComponentReferencer) (*TemplateContext, error) {
	...
	if d, ok := target.(Describable); ok {
		tplC.Component.Type = d.DescType()
		tplC.Component.Name = d.DescName()
	}
	if p, ok := target.(Parameterized); ok {
		tplC.Component.Params = CloneParameters(p.Params())
	}
	if p, ok := target.(ProxyAware); ok {
		tplC.Component.Proxy = p.ProxyInfo()
	}
	return &tplC, nil
}
